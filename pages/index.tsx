import ContentBlock from "@/components/ContentArticle";
import Container from "@/components/hocs/Container";
import HomeSvg from "@/components/svgs/HomeSvg";
import Link from "next/link";
import React from "react";
export default function Home() {
  // let d = "";
  return (
    <Container customAddClases="home page-template-default page page-id-152208 wp-embed-responsive ohio-theme-3-1-2 with-switcher with-header-3 with-fixed-search with-sticky-header with-mobile-switcher links-underline icon-buttons-animation custom-cursor wpb-js-composer js-comp-ver-6.10.0 vc_responsive elementor-default elementor-kit-6 page-is-loaded">
      <div
        id="content"
        className="site-content"
        data-mobile-menu-resolution={768}
      >
        <div className="page-container">
          <div id="primary" className="content-area">
            <div className="page-content">
              <main id="main" className="site-main">
                <article
                  id="post-152208"
                  className="post-152208 page type-page status-publish hentry"
                >
                  <div className="entry-content">
                    <section className="wpb-content-wrapper">
                      <div
                        className="vc_row wpb_row vc_row-fluid vc_row-o-full-height vc_row-o-columns-stretch vc_row-o-equal-height vc_row-o-content-bottom vc_row-flex"
                        style={{ minHeight: "100vh" }}
                      >
                        <div className="vc-bg-lines page-container -dark">
                          <div />
                          <div />
                          <div />
                          <div />
                          <div />
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-5">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space"
                                style={{ height: "5vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="vc_empty_space  vc_hidden-lg vc_hidden-md"
                                style={{ height: "5vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-widget heading -left aos-init aos-animate"
                                id="ohio-custom-63d3a0516f127"
                                data-aos="fade-up"
                              >
                                <h6 className="title">
                                  <span>We are proud of </span>
                                  <br className="vc_hidden-sm" />
                                  the works we’ve done.{" "}
                                </h6>
                              </div>
                              <div
                                className="vc_empty_space"
                                style={{ height: "2vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-widget heading clb__heading -left aos-init aos-animate"
                                id="ohio-custom-63d3a0516f614"
                                data-aos="fade-up"
                                data-aos-delay={100}
                              >
                                <h1
                                  className="title"
                                  style={{ fontSize: "4rem" }}
                                >
                                  We’re
                                  <br className="vc_hidden-xs vc_hidden-sm" />{" "}
                                  design creators.{" "}
                                </h1>
                              </div>
                              <div
                                className="vc_empty_space"
                                style={{ height: "3vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="ohio-widget-holder -left">
                                <a
                                  href="#"
                                  className="ohio-widget button -text aos-init aos-animate"
                                  id="ohio-custom-63d3a0516f77e"
                                  data-aos="fade-up"
                                  data-aos-delay={300}
                                >
                                  More Projects
                                </a>
                              </div>
                              <div
                                className="vc_empty_space  clb__section_spacer"
                                style={{ height: "18vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-1">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper" />
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-6">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div className="wpb_single_image wpb_content_element vc_align_center  vc_custom_1638976601881">
                                <figure className="wpb_wrapper vc_figure">
                                  <div className="vc_single_image-wrapper   vc_box_border_grey">
                                    <img
                                      width={1488}
                                      height={1560}
                                      src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1.png"
                                      className="vc_single_image-img attachment-full"
                                      alt=""
                                      decoding="async"
                                      loading="lazy"
                                      title="oh__demo1__1"
                                      srcSet="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1-286x300.png 286w, https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1-977x1024.png 977w, https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1-768x805.png 768w, https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1-1465x1536.png 1465w, https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__demo1__1.png 1488w"
                                      sizes="(max-width: 1488px) 100vw, 1488px"
                                    />
                                  </div>
                                </figure>
                              </div>
                              <div
                                className="ohio-text-sc aos-init aos-animate"
                                id="ohio-custom-63d3a05171904"
                                data-aos="fade-up"
                                data-aos-delay={400}
                              >
                                <p>Featured project:</p>
                              </div>
                              <div
                                className="ohio-widget heading -left aos-init aos-animate"
                                id="ohio-custom-63d3a05171cc5"
                                data-aos="fade-up"
                                data-aos-delay={500}
                              >
                                <h5 className="title">
                                  <a href="#demo1/project/diseno-grafico-scenic/">
                                    Unique Bowls From
                                    <br /> the South Pacific.
                                  </a>{" "}
                                </h5>
                              </div>
                              <div
                                className="vc_empty_space  clb__section_spacer"
                                style={{ height: "18vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>

                      <div
                        data-vc-full-width="true"
                        data-vc-full-width-init="true"
                        className="vc_row wpb_row vc_row-fluid vc_custom_1632207014191 vc_row-has-fill"
                        style={{
                          position: "relative",

                          boxSizing: "border-box",

                          maxWidth: "1349px",
                          paddingLeft: "10.5px",
                          paddingRight: "10.5px",
                        }}
                      >
                        <div className="wpb_column vc_column_container vc_col-sm-5">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-widget heading clb__heading -left"
                                id="ohio-custom-63d3a051724c0"
                              >
                                <div className="subtitle">Services</div>
                                <div className="divider" />
                                <h2 className="title">
                                  We're a team of creatives who are excited
                                  about unique ideas.
                                </h2>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-1">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper" />
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-3">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-12">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget heading -left"
                                        id="ohio-custom-63d3a05173690"
                                      >
                                        <div className="subtitle">Strategy</div>
                                        <div className="divider" />
                                        <h4 className="title">
                                          Brand Strategy &amp; Art Direction
                                        </h4>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a05173726"
                                      >
                                        <p>
                                          Creating a higher spacing and how
                                          people move through a unique and
                                          impactful campaign.
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div
                                className="vc_empty_space clb__spacer"
                                style={{ height: "6vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-12">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget heading -left"
                                        id="ohio-custom-63d3a05173ad5"
                                      >
                                        <div className="subtitle">
                                          Production
                                        </div>
                                        <div className="divider" />
                                        <h4 className="title">
                                          Typography &amp; Video Production
                                        </h4>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a05173b5b"
                                      >
                                        <p>
                                          Spaces of each debt in the digital
                                          world can help you with overall
                                          simplest authentic.
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div
                                className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-3">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-12">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget heading -left"
                                        id="ohio-custom-63d3a0517410d"
                                      >
                                        <div className="subtitle">Design</div>
                                        <div className="divider" />
                                        <h4 className="title">
                                          UX/UI Design &amp; Website Design
                                        </h4>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a051741b9"
                                      >
                                        <p>
                                          Get a regulatory oneven an enterprises
                                          such she and the got the did
                                          attributing and pushed.
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div
                                className="vc_empty_space clb__spacer"
                                style={{ height: "6vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-12">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget heading -left"
                                        id="ohio-custom-63d3a05174534"
                                      >
                                        <div className="subtitle">
                                          Campaigns
                                        </div>
                                        <div className="divider" />
                                        <h4 className="title">
                                          Promo Campaigns &amp; Content Creation
                                        </h4>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a051745b1"
                                      >
                                        <p>
                                          Making an email campaigns to achieve a
                                          real impact.
                                        </p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div
                                className="vc_empty_space clb__section_spacer clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="vc_row-full-width vc_clearfix" />
                      <div
                        data-vc-full-width="true"
                        data-vc-full-width-init="true"
                        data-vc-stretch-content="true"
                        className="vc_row wpb_row vc_row-fluid clb__dark_section"
                        style={{
                          position: "relative",

                          boxSizing: "border-box",

                          maxWidth: "1349px",
                        }}
                      >
                        <div className="wpb_column vc_column_container vc_col-sm-12">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="ohio-widget portfolio-projects clb__custom_portfolio grid_2"
                                id="ohio-custom-63d3a05174c4e"
                                data-ohio-portfolio-grid="true"
                              >
                                <div
                                  className="vc_row portfolio-grid"
                                  data-isotope-grid="true"
                                  data-lazy-container="projects"
                                  data-projects-per-page={4}
                                  style={{
                                    position: "relative",
                                    height: "1007.97px",
                                  }}
                                >
                                  <div
                                    className="portfolio-item-wrap masonry-block grid-item vc_col-lg-4 vc_col-md-4 vc_col-xs-12 ohio-filter-project-2ec9e9be8bed1ffe0ed8144a3f59c246 ohio-filter-project-70012063bf1d91c6d3924d1475e0747a"
                                    data-lazy-item
                                    data-lazy-scope="projects"
                                    style={{}}
                                  >
                                    <div
                                      className="portfolio-item card -layout2 -img-scale"
                                      data-portfolio-popup="ohio-lightbox-63d3a0517e6dc"
                                    >
                                      <div className="image-holder">
                                        <a
                                          className="-unlink"
                                          href="#demo1/project/stickers-pack/"
                                          data-cursor-class="cursor-link"
                                        >
                                          <img
                                            src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/zdfLZADL-oh__demo31__1.jpeg"
                                            alt="Stickers Pack"
                                          />
                                          <div className="overlay" />
                                        </a>
                                        <div className="overlay-details -top -fade-down">
                                          <button className="icon-button -light btn-lightbox">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 2V6H2V2H6V0H2C0.9 0 0 0.9 0 2ZM2 12H0V16C0 17.1 0.9 18 2 18H6V16H2V12ZM16 16H12V18H16C17.1 18 18 17.1 18 16V12H16V16ZM16 0H12V2H16V6H18V2C18 0.9 17.1 0 16 0Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.427734 1.20793C0.427734 0.77758 0.776603 0.428711 1.20696 0.428711H5.10306C5.53341 0.428711 5.88228 0.77758 5.88228 1.20793C5.88228 1.63828 5.53341 1.98715 5.10306 1.98715H1.98618V5.10404C1.98618 5.53439 1.63731 5.88326 1.20696 5.88326C0.776603 5.88326 0.427734 5.53439 0.427734 5.10404V1.20793ZM12.116 1.20793C12.116 0.77758 12.4649 0.428711 12.8953 0.428711H16.7914C17.2217 0.428711 17.5706 0.77758 17.5706 1.20793V5.10404C17.5706 5.53439 17.2217 5.88326 16.7914 5.88326C16.361 5.88326 16.0121 5.53439 16.0121 5.10404V1.98715H12.8953C12.4649 1.98715 12.116 1.63828 12.116 1.20793ZM1.20696 12.117C1.63731 12.117 1.98618 12.4659 1.98618 12.8962V16.0131H5.10306C5.53341 16.0131 5.88228 16.362 5.88228 16.7923C5.88228 17.2227 5.53341 17.5716 5.10306 17.5716H1.20696C0.776603 17.5716 0.427734 17.2227 0.427734 16.7923V12.8962C0.427734 12.4659 0.776603 12.117 1.20696 12.117ZM16.7914 12.117C17.2217 12.117 17.5706 12.4659 17.5706 12.8962V16.7923C17.5706 17.2227 17.2217 17.5716 16.7914 17.5716H12.8953C12.4649 17.5716 12.116 17.2227 12.116 16.7923C12.116 16.362 12.4649 16.0131 12.8953 16.0131H16.0121V12.8962C16.0121 12.4659 16.361 12.117 16.7914 12.117Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div
                                          className="video-button -animation open-popup"
                                          data-video="https://www.youtube.com/embed/t67_zAg5vvI?"
                                        >
                                          <button className="icon-button">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={13}
                                                height={20}
                                                viewBox="0 0 13 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 20L13 10L0 0V20Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={17}
                                                height={20}
                                                viewBox="0 0 17 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.766274 0.442678C0.998698 0.312329 1.26165 0.24625 1.52808 0.25124C1.79452 0.256229 2.05481 0.332105 2.28219 0.471065L15.78 8.72C15.9993 8.85399 16.1804 9.04206 16.3061 9.26618C16.4318 9.4903 16.4978 9.74295 16.4978 9.99991C16.4978 10.2569 16.4318 10.5095 16.3061 10.7336C16.1804 10.9578 15.9993 11.1458 15.78 11.2798L2.28219 19.5288C2.05481 19.6677 1.79451 19.7436 1.52808 19.7486C1.26165 19.7536 0.9987 19.6875 0.766274 19.5571C0.533848 19.4268 0.340346 19.2369 0.205669 19.0069C0.0709916 18.777 1.3411e-07 18.5153 0 18.2488V1.75098C1.3411e-07 1.48449 0.0709911 1.22282 0.205669 0.992883C0.340347 0.76294 0.533849 0.573027 0.766274 0.442678ZM14.9978 9.99991L1.5 1.75098L1.5 18.2488L14.9978 9.99991Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div className="overlay-details">
                                          <div className="heading">
                                            <h4 className="title">
                                              Stickers Pack
                                            </h4>
                                            <div className="show-project">
                                              <div className="category-holder">
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/identity/">
                                                    Identity
                                                  </a>
                                                </span>
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/mockup/">
                                                    Mockup
                                                  </a>
                                                </span>
                                              </div>
                                              <div className="show-project-link -full-w">
                                                <a href="#demo1/project/stickers-pack/">
                                                  Show project
                                                </a>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  <div
                                    className="portfolio-item-wrap masonry-block grid-item vc_col-lg-8 vc_col-md-8 vc_col-xs-12 double-width ohio-filter-project-5d845f00a32d05fb32beff8daf817d62 ohio-filter-project-2ec9e9be8bed1ffe0ed8144a3f59c246"
                                    data-lazy-item
                                    data-lazy-scope="projects"
                                    style={{}}
                                  >
                                    <div
                                      className="portfolio-item card -layout2 -img-scale"
                                      data-portfolio-popup="ohio-lightbox-63d3a0518508a"
                                    >
                                      <div className="image-holder">
                                        <a
                                          className="-unlink"
                                          href="#demo1/project/diseno-grafico/"
                                          data-cursor-class="cursor-link"
                                        >
                                          <img
                                            src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/oh__img25-1920x1076.jpeg"
                                            alt="Diseño Gráfico"
                                          />
                                          <div className="overlay" />
                                        </a>
                                        <div className="overlay-details -top -fade-down">
                                          <button className="icon-button -light btn-lightbox">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 2V6H2V2H6V0H2C0.9 0 0 0.9 0 2ZM2 12H0V16C0 17.1 0.9 18 2 18H6V16H2V12ZM16 16H12V18H16C17.1 18 18 17.1 18 16V12H16V16ZM16 0H12V2H16V6H18V2C18 0.9 17.1 0 16 0Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.427734 1.20793C0.427734 0.77758 0.776603 0.428711 1.20696 0.428711H5.10306C5.53341 0.428711 5.88228 0.77758 5.88228 1.20793C5.88228 1.63828 5.53341 1.98715 5.10306 1.98715H1.98618V5.10404C1.98618 5.53439 1.63731 5.88326 1.20696 5.88326C0.776603 5.88326 0.427734 5.53439 0.427734 5.10404V1.20793ZM12.116 1.20793C12.116 0.77758 12.4649 0.428711 12.8953 0.428711H16.7914C17.2217 0.428711 17.5706 0.77758 17.5706 1.20793V5.10404C17.5706 5.53439 17.2217 5.88326 16.7914 5.88326C16.361 5.88326 16.0121 5.53439 16.0121 5.10404V1.98715H12.8953C12.4649 1.98715 12.116 1.63828 12.116 1.20793ZM1.20696 12.117C1.63731 12.117 1.98618 12.4659 1.98618 12.8962V16.0131H5.10306C5.53341 16.0131 5.88228 16.362 5.88228 16.7923C5.88228 17.2227 5.53341 17.5716 5.10306 17.5716H1.20696C0.776603 17.5716 0.427734 17.2227 0.427734 16.7923V12.8962C0.427734 12.4659 0.776603 12.117 1.20696 12.117ZM16.7914 12.117C17.2217 12.117 17.5706 12.4659 17.5706 12.8962V16.7923C17.5706 17.2227 17.2217 17.5716 16.7914 17.5716H12.8953C12.4649 17.5716 12.116 17.2227 12.116 16.7923C12.116 16.362 12.4649 16.0131 12.8953 16.0131H16.0121V12.8962C16.0121 12.4659 16.361 12.117 16.7914 12.117Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div className="overlay-details">
                                          <div className="heading">
                                            <h4 className="title">
                                              Diseño Gráfico
                                            </h4>
                                            <div className="show-project">
                                              <div className="category-holder">
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/branding/">
                                                    Branding
                                                  </a>
                                                </span>
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/identity/">
                                                    Identity
                                                  </a>
                                                </span>
                                              </div>
                                              <div className="show-project-link -full-w">
                                                <a href="#demo1/project/diseno-grafico/">
                                                  Show project
                                                </a>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  <div
                                    className="portfolio-item-wrap masonry-block grid-item vc_col-lg-8 vc_col-md-8 vc_col-xs-12 double-width ohio-filter-project-123ae12c4c011e4cddeae3371b08bac6 ohio-filter-project-f5a27e0cf01635b6d1350ab621aabe25"
                                    data-lazy-item
                                    data-lazy-scope="projects"
                                    style={{}}
                                  >
                                    <div
                                      className="portfolio-item card -layout2 -img-scale"
                                      data-portfolio-popup="ohio-lightbox-63d3a0518789e"
                                    >
                                      <div className="image-holder">
                                        <a
                                          className="-unlink"
                                          href="#demo1/project/lifestyle-app/"
                                          data-cursor-class="cursor-link"
                                        >
                                          <img
                                            src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/drZ4vTCS-oh__img95-1920x1076.jpeg"
                                            alt="Lifestyle App"
                                          />
                                          <div className="overlay" />
                                        </a>
                                        <div className="overlay-details -top -fade-down">
                                          <button className="icon-button -light btn-lightbox">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 2V6H2V2H6V0H2C0.9 0 0 0.9 0 2ZM2 12H0V16C0 17.1 0.9 18 2 18H6V16H2V12ZM16 16H12V18H16C17.1 18 18 17.1 18 16V12H16V16ZM16 0H12V2H16V6H18V2C18 0.9 17.1 0 16 0Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.427734 1.20793C0.427734 0.77758 0.776603 0.428711 1.20696 0.428711H5.10306C5.53341 0.428711 5.88228 0.77758 5.88228 1.20793C5.88228 1.63828 5.53341 1.98715 5.10306 1.98715H1.98618V5.10404C1.98618 5.53439 1.63731 5.88326 1.20696 5.88326C0.776603 5.88326 0.427734 5.53439 0.427734 5.10404V1.20793ZM12.116 1.20793C12.116 0.77758 12.4649 0.428711 12.8953 0.428711H16.7914C17.2217 0.428711 17.5706 0.77758 17.5706 1.20793V5.10404C17.5706 5.53439 17.2217 5.88326 16.7914 5.88326C16.361 5.88326 16.0121 5.53439 16.0121 5.10404V1.98715H12.8953C12.4649 1.98715 12.116 1.63828 12.116 1.20793ZM1.20696 12.117C1.63731 12.117 1.98618 12.4659 1.98618 12.8962V16.0131H5.10306C5.53341 16.0131 5.88228 16.362 5.88228 16.7923C5.88228 17.2227 5.53341 17.5716 5.10306 17.5716H1.20696C0.776603 17.5716 0.427734 17.2227 0.427734 16.7923V12.8962C0.427734 12.4659 0.776603 12.117 1.20696 12.117ZM16.7914 12.117C17.2217 12.117 17.5706 12.4659 17.5706 12.8962V16.7923C17.5706 17.2227 17.2217 17.5716 16.7914 17.5716H12.8953C12.4649 17.5716 12.116 17.2227 12.116 16.7923C12.116 16.362 12.4649 16.0131 12.8953 16.0131H16.0121V12.8962C16.0121 12.4659 16.361 12.117 16.7914 12.117Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div
                                          className="video-button -animation open-popup"
                                          data-video="https://www.youtube.com/embed/t67_zAg5vvI?"
                                        >
                                          <button className="icon-button">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={13}
                                                height={20}
                                                viewBox="0 0 13 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 20L13 10L0 0V20Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={17}
                                                height={20}
                                                viewBox="0 0 17 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.766274 0.442678C0.998698 0.312329 1.26165 0.24625 1.52808 0.25124C1.79452 0.256229 2.05481 0.332105 2.28219 0.471065L15.78 8.72C15.9993 8.85399 16.1804 9.04206 16.3061 9.26618C16.4318 9.4903 16.4978 9.74295 16.4978 9.99991C16.4978 10.2569 16.4318 10.5095 16.3061 10.7336C16.1804 10.9578 15.9993 11.1458 15.78 11.2798L2.28219 19.5288C2.05481 19.6677 1.79451 19.7436 1.52808 19.7486C1.26165 19.7536 0.9987 19.6875 0.766274 19.5571C0.533848 19.4268 0.340346 19.2369 0.205669 19.0069C0.0709916 18.777 1.3411e-07 18.5153 0 18.2488V1.75098C1.3411e-07 1.48449 0.0709911 1.22282 0.205669 0.992883C0.340347 0.76294 0.533849 0.573027 0.766274 0.442678ZM14.9978 9.99991L1.5 1.75098L1.5 18.2488L14.9978 9.99991Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div className="overlay-details">
                                          <div className="heading">
                                            <h4 className="title">
                                              Lifestyle App
                                            </h4>
                                            <div className="show-project">
                                              <div className="category-holder">
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/apps/">
                                                    Apps
                                                  </a>
                                                </span>
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/campaign/">
                                                    Campaign
                                                  </a>
                                                </span>
                                              </div>
                                              <div className="show-project-link -full-w">
                                                <a href="#demo1/project/lifestyle-app/">
                                                  Show project
                                                </a>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  <div
                                    className="portfolio-item-wrap masonry-block grid-item vc_col-lg-4 vc_col-md-4 vc_col-xs-12 ohio-filter-project-6a81d94fbf5dcff590506b4b42e693bd ohio-filter-project-2ec9e9be8bed1ffe0ed8144a3f59c246"
                                    data-lazy-item
                                    data-lazy-scope="projects"
                                    style={{}}
                                  >
                                    <div
                                      className="portfolio-item card -layout2 -img-scale"
                                      data-portfolio-popup="ohio-lightbox-63d3a051897b4"
                                    >
                                      <div className="image-holder">
                                        <a
                                          className="-unlink"
                                          href="#demo1/project/credit-bank/"
                                          data-cursor-class="cursor-link"
                                        >
                                          <img
                                            src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/zxDVPhmF-oh__img31.jpeg"
                                            alt="Credit Bánk"
                                          />
                                          <div className="overlay" />
                                        </a>
                                        <div className="overlay-details -top -fade-down">
                                          <button className="icon-button -light btn-lightbox">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 2V6H2V2H6V0H2C0.9 0 0 0.9 0 2ZM2 12H0V16C0 17.1 0.9 18 2 18H6V16H2V12ZM16 16H12V18H16C17.1 18 18 17.1 18 16V12H16V16ZM16 0H12V2H16V6H18V2C18 0.9 17.1 0 16 0Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={18}
                                                height={18}
                                                viewBox="0 0 18 18"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.427734 1.20793C0.427734 0.77758 0.776603 0.428711 1.20696 0.428711H5.10306C5.53341 0.428711 5.88228 0.77758 5.88228 1.20793C5.88228 1.63828 5.53341 1.98715 5.10306 1.98715H1.98618V5.10404C1.98618 5.53439 1.63731 5.88326 1.20696 5.88326C0.776603 5.88326 0.427734 5.53439 0.427734 5.10404V1.20793ZM12.116 1.20793C12.116 0.77758 12.4649 0.428711 12.8953 0.428711H16.7914C17.2217 0.428711 17.5706 0.77758 17.5706 1.20793V5.10404C17.5706 5.53439 17.2217 5.88326 16.7914 5.88326C16.361 5.88326 16.0121 5.53439 16.0121 5.10404V1.98715H12.8953C12.4649 1.98715 12.116 1.63828 12.116 1.20793ZM1.20696 12.117C1.63731 12.117 1.98618 12.4659 1.98618 12.8962V16.0131H5.10306C5.53341 16.0131 5.88228 16.362 5.88228 16.7923C5.88228 17.2227 5.53341 17.5716 5.10306 17.5716H1.20696C0.776603 17.5716 0.427734 17.2227 0.427734 16.7923V12.8962C0.427734 12.4659 0.776603 12.117 1.20696 12.117ZM16.7914 12.117C17.2217 12.117 17.5706 12.4659 17.5706 12.8962V16.7923C17.5706 17.2227 17.2217 17.5716 16.7914 17.5716H12.8953C12.4649 17.5716 12.116 17.2227 12.116 16.7923C12.116 16.362 12.4649 16.0131 12.8953 16.0131H16.0121V12.8962C16.0121 12.4659 16.361 12.117 16.7914 12.117Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div
                                          className="video-button -animation open-popup"
                                          data-video="https://www.youtube.com/embed/t67_zAg5vvI?"
                                        >
                                          <button className="icon-button">
                                            <i className="icon">
                                              <svg
                                                className="default"
                                                width={13}
                                                height={20}
                                                viewBox="0 0 13 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path d="M0 20L13 10L0 0V20Z" />
                                              </svg>
                                              <svg
                                                className="minimal"
                                                width={17}
                                                height={20}
                                                viewBox="0 0 17 20"
                                                fill="none"
                                                xmlns="http://www.w3.org/2000/svg"
                                              >
                                                <path
                                                  fillRule="evenodd"
                                                  clipRule="evenodd"
                                                  d="M0.766274 0.442678C0.998698 0.312329 1.26165 0.24625 1.52808 0.25124C1.79452 0.256229 2.05481 0.332105 2.28219 0.471065L15.78 8.72C15.9993 8.85399 16.1804 9.04206 16.3061 9.26618C16.4318 9.4903 16.4978 9.74295 16.4978 9.99991C16.4978 10.2569 16.4318 10.5095 16.3061 10.7336C16.1804 10.9578 15.9993 11.1458 15.78 11.2798L2.28219 19.5288C2.05481 19.6677 1.79451 19.7436 1.52808 19.7486C1.26165 19.7536 0.9987 19.6875 0.766274 19.5571C0.533848 19.4268 0.340346 19.2369 0.205669 19.0069C0.0709916 18.777 1.3411e-07 18.5153 0 18.2488V1.75098C1.3411e-07 1.48449 0.0709911 1.22282 0.205669 0.992883C0.340347 0.76294 0.533849 0.573027 0.766274 0.442678ZM14.9978 9.99991L1.5 1.75098L1.5 18.2488L14.9978 9.99991Z"
                                                />
                                              </svg>
                                            </i>
                                          </button>
                                        </div>
                                        <div className="overlay-details">
                                          <div className="heading">
                                            <h4 className="title">
                                              Credit Bánk
                                            </h4>
                                            <div className="show-project">
                                              <div className="category-holder">
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/creative/">
                                                    Creative
                                                  </a>
                                                </span>
                                                <span className="category">
                                                  <a href="#demo1/portfolio-category/identity/">
                                                    Identity
                                                  </a>
                                                </span>
                                              </div>
                                              <div className="show-project-link -full-w">
                                                <a href="#demo1/project/credit-bank/">
                                                  Show project
                                                </a>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="vc_row-full-width vc_clearfix" />
                      <div
                        data-vc-full-width="true"
                        data-vc-full-width-init="true"
                        className="vc_row wpb_row vc_row-fluid vc_custom_1626683432877 vc_row-has-fill"
                        style={{
                          position: "relative",

                          boxSizing: "border-box",

                          maxWidth: "1349px",
                          paddingLeft: "10.5px",
                          paddingRight: "10.5px",
                        }}
                      >
                        <div className="wpb_column vc_column_container vc_col-sm-5">
                          <div className="vc_column-inner vc_custom_1626683807456">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-widget heading -left"
                                id="ohio-custom-63d3a0518a61d"
                              >
                                <div className="subtitle">Capabilities</div>
                                <div className="divider" />
                                <h2 className="title">
                                  Creative studio with art and technologies.
                                </h2>
                              </div>
                              <div
                                className="vc_empty_space vc_hidden-lg"
                                style={{ height: "5vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-7">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper" />
                          </div>
                        </div>
                      </div>
                      <div className="vc_row-full-width vc_clearfix" />

                      <div
                        data-vc-full-width="true"
                        data-vc-full-width-init="true"
                        className="vc_row wpb_row vc_row-fluid vc_custom_1626264752003 vc_row-has-fill"
                        style={{
                          position: "relative",

                          boxSizing: "border-box",

                          maxWidth: "1349px",
                          paddingLeft: "10.5px",
                          paddingRight: "10.5px",
                        }}
                      >
                        <div className="wpb_column vc_column_container vc_col-sm-12">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211602980">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a0518d8be"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211607830">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a0518e975"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-2-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211612193">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a0518f1f8"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-4-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211617065">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a0518fac0"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-6-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211724324">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a051902df"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-8-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211625299">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a05190b6b"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-10-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                      <div
                                        className="vc_empty_space vc_hidden-xs vc_hidden-sm"
                                        style={{ height: "20px" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211602980">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a05191677"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-1-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211607830">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a05191dde"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-3-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211612193">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a051924cb"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-5-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211617065">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a05192c78"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-7-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211724324">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a051933df"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-9-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-2 vc_col-xs-6">
                                  <div className="vc_column-inner vc_custom_1571211625299">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget logo -left"
                                        id="ohio-custom-63d3a05193af4"
                                      >
                                        <a
                                          className="-unlink"
                                          href="#demo1/#"
                                        >
                                          <img
                                            src="./Ohio Stage Demo 1 – WordPress theme demo site_files/LogoSC-11-min.png"
                                            sizes="(max-width: 268px) 100vw, 268px"
                                            alt=""
                                          />
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="vc_row-full-width vc_clearfix" />

                      <div className="vc_row wpb_row vc_row-fluid">
                        <div className="wpb_column vc_column_container vc_col-sm-5">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-widget heading -left"
                                id="ohio-custom-63d3a0519a0f7"
                              >
                                <div className="subtitle">
                                  Awards &amp; Honors
                                </div>
                                <div className="divider" />
                                <h3 className="title">
                                  The awards won
                                  <br />
                                  by our project.
                                </h3>
                              </div>
                              <div
                                className="vc_empty_space"
                                style={{ height: "3vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-1">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper" />
                          </div>
                        </div>
                        <div className="wpb_column vc_column_container vc_col-sm-6">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer vc_hidden-xs vc_hidden-sm"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div
                                className="ohio-text-sc"
                                id="ohio-custom-63d3a0519a692"
                              >
                                <p>
                                  Our clients describe us as a product team
                                  which
                                  <strong>creates amazing UI/UX</strong>
                                  experiences,
                                  <strong>by crafting top-notch</strong> user
                                  experience.
                                </p>
                              </div>
                              <div
                                className="vc_empty_space clb__spacer"
                                style={{ height: "6vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-4 vc_col-xs-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div className="wpb_single_image wpb_content_element vc_align_left vc_custom_1638974696470">
                                        <figure className="wpb_wrapper vc_figure">
                                          <div className="vc_single_image-wrapper vc_box_border_grey">
                                            <img
                                              width={352}
                                              height={96}
                                              src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/award_awwwards-min.png"
                                              className="vc_single_image-img attachment-full"
                                              alt=""
                                              decoding="async"
                                              loading="lazy"
                                              title="award_awwwards-min"
                                              sizes="(max-width: 352px) 100vw, 352px"
                                            />
                                          </div>
                                        </figure>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519b386"
                                      >
                                        <p>
                                          <strong>5x</strong> Developer Award
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space"
                                        style={{ height: "5vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                      <div className="wpb_single_image wpb_content_element vc_align_left vc_custom_1638974715157">
                                        <figure className="wpb_wrapper vc_figure">
                                          <div className="vc_single_image-wrapper vc_box_border_grey">
                                            <img
                                              width={352}
                                              height={96}
                                              src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/award_awwwards-min.png"
                                              className="vc_single_image-img attachment-full"
                                              alt=""
                                              decoding="async"
                                              loading="lazy"
                                              title="award_webby-min"
                                              sizes="(max-width: 352px) 100vw, 352px"
                                            />
                                          </div>
                                        </figure>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519bc14"
                                      >
                                        <p>
                                          <strong>2x</strong> Best Website
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space clb__spacer"
                                        style={{ height: "10vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-4 vc_col-xs-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div className="wpb_single_image wpb_content_element vc_align_left vc_custom_1638974696470">
                                        <figure className="wpb_wrapper vc_figure">
                                          <div className="vc_single_image-wrapper vc_box_border_grey">
                                            <img
                                              width={352}
                                              height={96}
                                              src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/award_awwwards-min.png"
                                              className="vc_single_image-img attachment-full"
                                              alt=""
                                              decoding="async"
                                              loading="lazy"
                                              title="award_awwwards-min"
                                              sizes="(max-width: 352px) 100vw, 352px"
                                            />
                                          </div>
                                        </figure>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519c442"
                                      >
                                        <p>
                                          <strong>3x</strong> Site of the Day
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space"
                                        style={{ height: "5vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                      <div className="wpb_single_image wpb_content_element vc_align_left vc_custom_1638974715157">
                                        <figure className="wpb_wrapper vc_figure">
                                          <div className="vc_single_image-wrapper vc_box_border_grey">
                                            <img
                                              width={352}
                                              height={96}
                                              src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/award_awwwards-min.png"
                                              className="vc_single_image-img attachment-full"
                                              alt=""
                                              decoding="async"
                                              loading="lazy"
                                              title="award_webby-min"
                                              sizes="(max-width: 352px) 100vw, 352px"
                                            />
                                          </div>
                                        </figure>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519caad"
                                      >
                                        <p>
                                          <strong>2x</strong> Best Website
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space"
                                        style={{ height: "5vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-4 vc_col-xs-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div className="wpb_single_image wpb_content_element vc_align_left vc_custom_1638974706944">
                                        <figure className="wpb_wrapper vc_figure">
                                          <div className="vc_single_image-wrapper vc_box_border_grey">
                                            <img
                                              width={352}
                                              height={96}
                                              src="https://colabrio.ams3.cdn.digitaloceanspaces.com/ohio-stage-demo-1/award_awwwards-min.png"
                                              className="vc_single_image-img attachment-full"
                                              alt=""
                                              decoding="async"
                                              loading="lazy"
                                              title="award_fwa-min"
                                              sizes="(max-width: 352px) 100vw, 352px"
                                            />
                                          </div>
                                        </figure>
                                      </div>
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519d5b4"
                                      >
                                        <p>
                                          <strong>5x</strong> Mobile of the Day
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space"
                                        style={{ height: "6vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        data-vc-full-width="true"
                        data-vc-full-width-init="true"
                        className="vc_row wpb_row vc_row-fluid vc_custom_1626696386753 vc_row-has-fill clb__dark_section"
                        style={{
                          position: "relative",

                          boxSizing: "border-box",

                          maxWidth: "1349px",
                          paddingLeft: "10.5px",
                          paddingRight: "10.5px",
                        }}
                      >
                        <div className="wpb_column vc_column_container vc_col-sm-12">
                          <div className="vc_column-inner">
                            <div className="wpb_wrapper">
                              <div
                                className="vc_empty_space clb__section_spacer"
                                style={{ height: "10vh" }}
                              >
                                <span className="vc_empty_space_inner" />
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-widget heading -left"
                                        id="ohio-custom-63d3a0519e088"
                                      >
                                        <div className="subtitle">
                                          Collaboration
                                        </div>
                                        <h2 className="title">
                                          Got a project?
                                          <br />
                                          Let’s talk.
                                        </h2>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="vc_empty_space"
                                        style={{ height: "3vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                      <div className="ohio-widget-holder -right">
                                        <a
                                          href="#contact-us-ver4/"
                                          target="_blank"
                                          className="ohio-widget button -primary clb__alignment_reset"
                                          id="ohio-custom-63d3a0519e445"
                                        >
                                          Contact Us
                                          <i className="icon -right">
                                            <svg
                                              className="default"
                                              width={16}
                                              height={16}
                                              viewBox="0 0 16 16"
                                              xmlns="http://www.w3.org/2000/svg"
                                            >
                                              <path d="M8 0L6.59 1.41L12.17 7H0V9H12.17L6.59 14.59L8 16L16 8L8 0Z" />
                                            </svg>
                                            <svg
                                              className="minimal"
                                              width={18}
                                              height={16}
                                              viewBox="0 0 18 16"
                                              fill="none"
                                              xmlns="http://www.w3.org/2000/svg"
                                            >
                                              <path
                                                fillRule="evenodd"
                                                clipRule="evenodd"
                                                d="M0 8C0 7.58579 0.335786 7.25 0.75 7.25H17.25C17.6642 7.25 18 7.58579 18 8C18 8.41421 17.6642 8.75 17.25 8.75H0.75C0.335786 8.75 0 8.41421 0 8Z"
                                              />
                                              <path
                                                fillRule="evenodd"
                                                clipRule="evenodd"
                                                d="M9.96967 0.71967C10.2626 0.426777 10.7374 0.426777 11.0303 0.71967L17.7803 7.46967C18.0732 7.76256 18.0732 8.23744 17.7803 8.53033L11.0303 15.2803C10.7374 15.5732 10.2626 15.5732 9.96967 15.2803C9.67678 14.9874 9.67678 14.5126 9.96967 14.2197L16.1893 8L9.96967 1.78033C9.67678 1.48744 9.67678 1.01256 9.96967 0.71967Z"
                                              />
                                            </svg>
                                          </i>
                                        </a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div className="vc_separator wpb_content_element vc_separator_align_center vc_sep_width_100 vc_sep_pos_align_center vc_separator_no_text">
                                <span className="vc_sep_holder vc_sep_holder_l">
                                  <span
                                    style={{
                                      borderColor: "rgba(99, 93, 111, 0.65)",
                                    }}
                                    className="vc_sep_line"
                                  />
                                </span>
                                <span className="vc_sep_holder vc_sep_holder_r">
                                  <span
                                    style={{
                                      borderColor: "rgba(99, 93, 111, 0.65)",
                                    }}
                                    className="vc_sep_line"
                                  />
                                </span>
                              </div>
                              <div className="vc_row wpb_row vc_inner vc_row-fluid">
                                <div className="wpb_column vc_column_container vc_col-sm-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper">
                                      <div
                                        className="ohio-text-sc"
                                        id="ohio-custom-63d3a0519e9d5"
                                      >
                                        <p>
                                          We’re
                                          <strong>a team of creatives</strong>
                                          who are excited about unique ideas and
                                          help fin-tech companies to
                                          <strong>
                                            create amazing identity
                                          </strong>
                                          by crafting top-notch UI/UX.
                                        </p>
                                      </div>
                                      <div
                                        className="vc_empty_space clb__section_spacer"
                                        style={{ height: "10vh" }}
                                      >
                                        <span className="vc_empty_space_inner" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="wpb_column vc_column_container vc_col-sm-6">
                                  <div className="vc_column-inner">
                                    <div className="wpb_wrapper" />
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="vc_row-full-width vc_clearfix" />
                    </section>
                  </div>
                </article>
              </main>
            </div>
          </div>
        </div>
      </div>
    </Container>
  );
}
